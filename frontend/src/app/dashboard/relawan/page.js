...
export default function DashboardRelawan() {
  ...
  const filteredReqs = useMemo(() =>
    reqFilter === 'all' ? requests : requests.filter(r => r.mode === reqFilter),
    [requests, reqFilter]
  );
  ...
  function acceptReq(id) {
    ...
    showToast(`Sesi dengan ${r?.nama} berhasil dikonfirmasi!`);
  }
        ...
        <div className={styles.sidebarFooter}>
          <div className={styles.userInfo}>
            <div className={styles.avWrap}>
              <div className={styles.av} style={{background:'#D85A30'}}>AP</div>
          ...
          <span className={`${styles.onlineLabel} ${isOnline ? styles.olOn : styles.olOff}`}>
          ...
          <div className={styles.pane}>
            <header className={styles.topBar}>
              <div>
                ...
                <p className={styles.pageDesc}>Senin, 21 Apr 2026 · {TODAY_SESSIONS.length} sesi hari ini</p>
              </div>
            ...
            <div className={styles.grid2}>
                ...
                {TODAY_SESSIONS.map((s, i) => (
                  <div key={i} className={styles.todayCard}>
                    <div className={styles.timeBlock} style={{background: i===0?'var(--coral-light)':'var(--teal-light)'}}>
                      <p className={styles.timeVal} style={{color: i===0?'var(--coral)':'var(--teal)'}}>{s.jam}</p>
                      <p className={styles.timeWib} style={{color: i===0?'#F0997B':'#5DCAA5'}}>WIB</p>
                    </div>
                    <div className={styles.todayBody}>
                      <p className={styles.todayMapel}>{s.mapel} — Sesi {s.sesi}</p>
                      <p className={styles.todayMeta}>{s.mode} • {s.jam}–{s.end}</p>
                      ...
                      <a href={`https://${s.gmeet}`} target="_blank" rel="noreferrer" className={styles.meetLink}>▶ Buka Meet</a>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className={styles.card}>
                ...
                {requests.slice(0,2).map((r, i) => (
                  <div key={r.id} className={styles.reqMiniCard}>
                    <div className={styles.av} style={{background: avColor(i), width:32, height:32, fontSize:10}}>{getInitials(r.nama)}</div>
                    <div className={styles.reqMiniInfo}>
                      <p className={styles.rn}>{r.nama}</p>
                      <div style={{display:'flex',alignItems:'center',gap:5}}><span className={styles.rxSmall}>{r.mapel}</span><ModeTag mode={r.mode} /></div>
                    </div>
                    <div className={styles.btnRow}>
                      <button className={styles.btnDecline} onClick={() => declineReq(r.id)}>Tolak</button>
                      <button className={styles.btnAccept} onClick={() => acceptReq(r.id)}>Terima</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {filteredReqs.length === 0 ? (
                ...
                <div key={r.id} className={styles.reqCard}>
                  <div className={styles.reqTop}>
                    <div className={styles.av} style={{background: avColor(i), width:36, height:36}}>{getInitials(r.nama)}</div>
                    <div className={styles.reqInfo}>
                      <p className={styles.rn}>{r.nama}</p>
                      <p className={styles.rx}>{r.mapel} • Usia {r.usia} • 📍 {r.kota}</p>
                    </div>
                    <ModeTag mode={r.mode} />
                  </div>
                  <div className={styles.reqBot}>
                    <span className={styles.reqTime}>📅 {r.tgl} • ⏰ {r.jam} WIB</span>
                    <div className={styles.btnRow}>
                      <button className={styles.btnDecline} onClick={() => declineReq(r.id)}>Tolak</button>
                      <button className={styles.btnAccept}  onClick={() => acceptReq(r.id)}>Terima</button>
                    </div>
